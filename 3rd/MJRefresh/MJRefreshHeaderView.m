...
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        // 2.加载时间
        self.lastUpdateTime = [[NSUserDefaults standardUserDefaults] objectForKey:@"MJRefreshHeaderView"];
    }
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = NSLocalizedString(@"LOC_TABLE_REFRESH_SLIDE_DOWN", nil);
        self.releaseToRefreshText = NSLocalizedString(@"LOC_TABLE_REFRESH_RELEASE", nil);
        self.refreshingText = NSLocalizedString(@"LOC_TABLE_REFRESH_LOADING", nil);
    }
    return self;
}
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    // 1.归档
    [[NSUserDefaults standardUserDefaults] setObject:lastUpdateTime forKey:@"MJRefreshHeaderView"];
